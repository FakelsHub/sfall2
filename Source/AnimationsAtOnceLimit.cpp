...
void AnimationsAtOnceInit() {
 ...
 if (AnimationsLimit > 32) {
  ...
  anim_set = new BYTE[2656*(AnimationsLimit+1)];// + пустышка
  sad = new BYTE[3240*(AnimationsLimit-8)];

  //reserved animation limit check (old 20)
  SafeWrite8(0x413C07, AnimationsLimit-12);

  //movement animation limit checks (old 24)
  for (i = 0; i < sizeof(AnimMove)/4; i++) {
   SafeWrite8(AnimMove[i], AnimationsLimit-8);
  }
