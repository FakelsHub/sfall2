...
void AnimationsAtOnceInit() {
 ...
 //allocate memory to store larger animation struct arrays + пустышка
 anim_set = new BYTE[2656*(AnimationsLimit+1)];
 sad = new BYTE[3240*(AnimationsLimit+1)];

 //set general animation limit check (old 20) AnimationsLimit-12 -- +12 reserved for PC movement(4) + other critical animations(8)?
 SafeWrite8(0x413C07, AnimationsLimit-12);

 //PC movement animation limit checks (old 24) AnimationsLimit-8 -- +8 reserved for other critical animations?.
 for (i = 0; i < sizeof(AnimPCMove)/4; i++) {
  SafeWrite8(AnimPCMove[i], AnimationsLimit-8);
 }

 //Max animation limit checks (old 32) AnimationsLimit
 for (i = 0; i < sizeof(AnimMaxCheck)/4; i++) {
  SafeWrite8(AnimMaxCheck[i], AnimationsLimit);
 }

 //Max animations checks - animation struct size * max num of animations (old 2656*32=84992)
 for (i = 0; i < sizeof(AnimMaxSizeCheck)/4; i++) {
  SafeWrite32(AnimMaxSizeCheck[i], 2656*AnimationsLimit);
 }
